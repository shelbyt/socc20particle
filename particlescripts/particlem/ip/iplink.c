...
int iplink_particle_add(int cmd, unsigned int flags, int argc, char **argv){
    ...
    //IP LINK ADD
    for (i = 0; i < 1000; i ++) {
        //The second set of argc,argv gets rid of the "add"
        memset(local,0,sizeof local);
        memset(guest,0,sizeof guest);

        sprintf(local, "vl%d",i);
        sprintf(guest, "vg%d",i);
...
int iplink_particle_s5(int cmd, unsigned int flags, int argc, char **argv){
    ...
    for (i = 0; i < 1000; i ++) {
        ...
        sprintf(local, "vg%d",i);
        sprintf(mac, "84:00:00:00:%x:%x",hex_in,hex_out);
...
int iplink_particle_s7(int cmd, unsigned int flags, int argc, char **argv){
...
int iplink_particle_s2(int cmd, unsigned int flags, int argc, char **argv){
...
int do_iplink(int argc, char **argv)
{
	...
	if (iplink_have_newlink()) {
		if (matches(*argv, "s1particle") == 0)
            //Doing argc-1 and argv+1 here because we'll run it as:
            //ip link PARTICLE add name ...etc. So get rid of particle
			return iplink_particle_add(RTM_NEWLINK,
					     NLM_F_CREATE|NLM_F_EXCL,
					     argc-1, argv+1);
		if (matches(*argv, "s2particle") == 0)
            //Doing argc-1 and argv+1 here because we'll run it as:
            //ip link PARTICLE set name ...etc. So get rid of particle
			return iplink_particle_s2(RTM_NEWLINK, 0,
					     argc-1, argv+1);
		if (matches(*argv, "s5particle") == 0)
            //Doing argc-1 and argv+1 here because we'll run it as:
            //ip link PARTICLE set name ...etc. So get rid of particle
			return iplink_particle_s5(RTM_NEWLINK, 0,
					     argc-1, argv+1);
		if (matches(*argv, "s7particle") == 0)
            //Doing argc-1 and argv+1 here because we'll run it as:
            //ip link PARTICLE set name ...etc. So get rid of particle
			return iplink_particle_s7(RTM_NEWLINK, 0,
					     argc-1, argv+1);
