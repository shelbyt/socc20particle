...
static int do_particle_switch(void *arg)
{
	char *netns = arg;
	...
	return netns_particle_switch(netns);
}
...
int netns_particle_exec(int argc, char **argv){
	 ...
	 */
	//return -cmd_particle_exec(argv[1], argv + 1, !!batch_mode, do_particle_switch, argv[0]);
	return -cmd_exec(argv[1], argv + 1, !!batch_mode, do_particle_switch, argv[0]);
...
int do_netns(int argc, char **argv)
{
	...
	if (argc > 1 && invalid_name(argv[1]) && !matches(*argv, "particle")) {
	...
	if (matches(*argv, "particle") == 0)
		return netns_particle_exec(argc-1, argv+1);
