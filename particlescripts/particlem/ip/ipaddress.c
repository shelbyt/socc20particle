...
static int ipaddr_s6particle_modify(int cmd, int flags, int argc, char **argv){
    int i = 0;
    int ip_in = 20;
    int ip_out = 2;
    char ip[24];
   
   // ip addr particle 100 1 add .... 
   
    int container_ips=strtol(argv[0],NULL,10);
    int node_id=strtol(argv[1],NULL,10);

    printf("container_ips = %d\n",container_ips);
    printf("nodeid = %d\n",node_id);
    ...
    //192.168.2(1).x
    ip_in = ip_in + node_id;
    //+1 so that it matche with ip pool, else it onyl creates 9 addresses
    for(i = 1; i < container_ips+1; i++){
        //memset(guest,0,sizeof guest);
        memset(ip,0,sizeof ip);

        //sprintf(guest,"vgd%d",i);
        sprintf(ip,"192.168.%d.%d/16",ip_in,ip_out);
		...
		//ipaddr_modify(RTM_NEWADDR, NLM_F_CREATE|NLM_F_EXCL, argc-1, argv+1);
		// 3 because adding 2 new args
		ipaddr_modify(RTM_NEWADDR, NLM_F_CREATE|NLM_F_EXCL, argc-3, argv+3);
    }
...
int do_ipaddr(int argc, char **argv)
{
    ...
    //Variant of the add with particle
	if (matches(*argv, "s6particle") == 0)
		return ipaddr_s6particle_modify(RTM_NEWADDR, NLM_F_CREATE|NLM_F_EXCL, argc-1, argv+1);
